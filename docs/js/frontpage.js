...
function fileRead(contents){
    ...
    if(frameDict){
        ...
        var panel1 = initFingerprintPanel("#maincontent", columnNames, threeStateSelection, true, '110px');

        // Precomputed Fingerprints
        var fingerprint_list = calcFingerprints(contents);
        var panel2 = initScrollableFingerprintPanel("#maincontent", columnNames, fingerprint_list, threeStateRowSelection, false, '0px');

        panel1.on("click", updateIntersectFrames)
        panel2.on("click", updateIntersectFrames)
    } else {
